...
4. The wrapper reserves/commits entitlement itself. Pass `-FeatureMode reference_reconstruction` for reference work or `-FeatureMode scientific_asset_drawing` for asset drawing. Reuse the same UsageId across revisions and apps. `-DryRun` validates without app access or creating output. Per-object delayed playback is not supported.
