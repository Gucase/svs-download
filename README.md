...
SVS 提供两个相互独立的绘图入口：**参考图重建**与**科研图元绘制**。两项功能都可以把结果同步到 Adobe Illustrator 或 PowerPoint，并保留图形和文字的可编辑性。

## 功能一｜参考图重建

SVS 根据参考图重建可编辑的图形和文字，保留布局、颜色及所需的渐变和透明度，并根据用户选择，将绘制结果同步到 Adobe Illustrator 或 PowerPoint，供查看和继续编辑。

支持 PNG、JPEG、TIFF 或 WebP 科研示意图。SVS 会测量原图布局与细节，再直接构建路径、形状和可编辑文字，不是把位图简单装进 SVG。

## 功能二｜科研图元绘制

不提供参考图时，也可以直接描述需要的医学、生物学、实验器皿或科研仪器。SVS 会从独立绘制的科研图元库中选择合适对象，或按科研规范补充缺少的图元，再在 Illustrator 或 PowerPoint 中组合成可编辑矢量图。

可绘制箭头、细胞与细胞器、常用器皿、医学结构和实验室仪器等。对于 HPLC、Q-TOF 等仪器，可根据需求采用清晰示意风格或更接近真实设备的写实矢量风格；不仿制具体厂商商标和受保护外观。
...
1. 自行打开 Illustrator 或 PowerPoint 及目标文档。
2. 说明要绘制的对象、用途、风格和目标应用；无需上传参考图。
3. 让 Codex 使用 SVS 的科研图元绘制功能完成原生可编辑矢量，并在目标应用中查看。

例如：

> 使用 SVS 的科研图元绘制功能，在我已经打开的 PowerPoint 中绘制一台写实风格的 HPLC 仪器，所有部件保持可编辑。
...
*注：页面展示的是 PowerPoint 原生可编辑矢量图的 PNG 效果预览，PNG 本身不是矢量文件。*

## 免费体验与买断

- 参考图重建免费体验 1 张；科研图元绘制免费体验 2 张，两项额度分别计算。
- 同一原图和科研目标下的技术失败重试、对照原图纠错，以及同步到 Illustrator/PPT，不重复占用免费次数。
- 更换原图、增加新的科研内容/面板，或制作实质不同的构图，试用期间按一张新图计次。
- **39 元一次买断，绑定一台电脑；一个授权文件同时解锁参考图重建与科研图元绘制，两项均不限次数，同机 Illustrator／PowerPoint 共用。**
...
| 方案 | 价格 | 绘图次数 |
|---|---:|---|
| 免费体验 | 0 元 | 参考图重建 1 张；科研图元绘制 2 张 |
| 个人买断 | 39 元 | 授权一台电脑，两项功能均不限次 |
